...
#pragma region UDP Client
	bool UDPClient::Connect(const std::string& ip)
	{
		return Connect(IPEndpoint(ip.c_str()));
	}

	bool UDPClient::Connect(IPEndpoint ip)
	{
		isConnected = false;

		UDPSocket socket = UDPSocket(ip.GetIPVersion());
		//Create the socket
		if (socket.Create() == GResult::G_SUCCESS)
		{
			//Set client to use blocking socket whilst server is under development
	...
	bool UDPClient::ProcessPacket(std::shared_ptr<Packet> packet)
	{
		std::cout << "Packet Received with size: " << packet->buffer.size() << std::endl;
		return true;
	}
	...
	void UDPClient::OnConnectFail()
	{
		std::cout << "Failed to connect." << std::endl;
	}

	void UDPClient::OnDisconnect(std::string reason)
	{
		std::cout << "Loct connection. Reason: " << reason << "." << std::endl;
	}
	void UDPClient::CloseConnection(std::string reason)
	{
		OnDisconnect(reason);
		master_fd.fd = 0;
		isConnected = false;
